    ...
    def add_node(self):
        ...
        if not self.collision(xnear, xint):
            ...
            if self.is_goal(xint):
                ...
                print('goal id: ', self.goalidx)

                # if self.goalidx != len(self.x1) - 1:
                #     goal = np.array([(self.x1[self.goalidx], self.x2[self.goalidx], self.x3[self.goalidx], self.x4[self.goalidx], self.x5[self.goalidx], self.x6[self.goalidx], self.x7[self.goalidx])])
...
if __name__ == "__main__":
    ...
    RRT_Instance = Informed_RRT_star(start, np.array(goal), goalrad)
