# -*- coding: utf-8 -*-
"""
Created on Thu Apr 13 19:06:18 2023
...
class Informed_RRT_star:
    ...
    def sample_envir(self):
        ...
        a = self.ellipsoid['axes']
        startpoint_coords = self.ellipsoid['start']
        endpoint_coords = self.ellipsoid['end']
        # Compute the transformation matrix T
        A = np.diag(a)
        U, s, Vt = np.linalg.svd(A)
        T = U / s
        
        # Define the coordinates of the endpoint of the longest axis in the transformed system
        
        startpoint_transformed = np.matmul(T, startpoint_coords)
        endpoint_transformed = np.matmul(T, endpoint_coords)
        
        
        # Sample a point within the unit sphere in the transformed system
        v_transformed = np.random.uniform(0, 1, shape=(1,7))
        v_transformed /= np.linalg.norm(v_transformed)
        
        # Apply the inverse transformation to obtain the sampled point in the original system
        v = np.matmul(np.linalg.inv(T) , v_transformed)
        
        # Scale the sampled point by the lengths of the semi-axes to obtain a point within the ellipsoid
        point = np.matmul(a, v)
